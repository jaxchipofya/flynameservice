...
contract Domains is ERC721URIStorage{
	...
	modifier isDomainOwner(string calldata name) {
		require(msg.sender==domains[name]);
		_;
	}

	modifier isZeroAddress(string calldata name) {
	...
	constructor(string memory _tld) payable ERC721("Fly Name Service","FNS") {
	...
	function register(string calldata name) public payable isZeroAddress(name) {
		...
		uint256 _length = StringUtils.strlen(name);
		string memory _strlen = Strings.toString(_length);
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Fly name service", "image": "data:image/svg+xml;base64,',
		...
		string memory finalTokenURI = string(abi.encodePacked("data:application/json;base64,",_json));
		console.log("\n--------------------------------------------------------");
	...
	function withdraw() public onlyOwner {
		...
		require(_success,"Failed to withdraw matic");
	}
	...
	function setRecord(string calldata name,string calldata record) public isDomainOwner(name) {
